...
character = P matcher
  where
    matcher (x:.xs) = Result xs x
    matcher Nil = UnexpectedEof
-- | Parsers can map.
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) (P a) (P b) = P(\input -> let par = a input in if isErrorResult par then b input else par)
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) f (P p) = P(\input -> onResult (p input) (\rest a -> parse (f a) rest))
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) (P pab) (P pa) = P(\input -> onResult (pab input) (\i2 f -> f <$> pa i2))
-- Input -> ParseResult a
...
list1 p = do
    li <- list p
    case li of Nil -> constantParser UnexpectedEof
               _   -> pure li
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\pa pla -> (:.) <$> pa <*> pla) (pure Nil)
...
personParser = do
  ...
  spaces1
  phone <- phoneParser
  pure (Person age first sur smoke phone)
