...
option ::
  ...
  -> Parser a
  -> Parser a
option va pa = tok pa ||| valueParser va
...
hex ::
  Parser Char
hex = (chr . f) <$> (replicateA 4 (satisfy isHexDigit))
  where f x = case readHex x of Empty -> 0
                                Full n -> n

  -- (Int -> Char) . (Chars -> Int a) <$> Parser (List Char)
  -- (List Char -> Char)
...
eof ::
  Parser ()
eof = P (\x -> if x == Nil then Result Nil () else UnexpectedEof)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll lb = satisfy (and . sequence lb)

-- sequence :: f => List (f a) -> f (List a)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny lb = satisfy (or . sequence lb)
